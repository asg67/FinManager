...
_FOOTER_BOILERPLATE_RE = re.compile(
    ...
    r"Для проверки подлинности|отсканируйте QR|Получите документ|"
    r"Действителен|Предоставляя QR|Дата формирования|"
    r"Генеральная лицензия|ПАО Сбербанк|Страница \d+\s*из\s*\d+|"
    r"Дата обработки|код авторизации|СВЕДЕНИЯ О СЕРТИФИКАТЕ|"
    r"Нажмите кнопку|Денежные средства списываются|"
    r"Дата списания|По курсу банка|Сертификат:|Владелец:|Дата подписи:)",
    re.I,
)
...
def _clean_purpose(text: str) -> str:
    ...
    # Strip leading auth codes (6-digit numbers) left after date removal
    t = re.sub(r"^\s*\d{4,6}\s+", "", t)
    # Strip certificate hashes and trailing junk
    t = re.sub(r"\*?\s*[0-9A-Fa-f]{16,}.*$", "", t)
    return _norm(t)
