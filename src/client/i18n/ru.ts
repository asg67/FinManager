export default {
  common: {
    loading: "Загрузка...",
    close: "Закрыть",
    email: "Email",
    save: "Сохранить",
    cancel: "Отмена",
    delete: "Удалить",
    edit: "Редактировать",
    confirmDelete: "Подтвердите удаление",
    select: "Выберите...",
    add: "Добавить",
    next: "Далее",
    back: "Назад",
    copy: "Копировать",
    sending: "Отправка...",
    error: "Ошибка",
  },
  auth: {
    login: "Вход",
    loginAction: "Войти",
    loginLoading: "Вход...",
    register: "Регистрация",
    registerAction: "Зарегистрироваться",
    registerLoading: "Регистрация...",
    password: "Пароль",
    confirmPassword: "Подтверждение пароля",
    lastName: "Фамилия",
    lastNamePlaceholder: "Ваша фамилия",
    firstName: "Имя",
    firstNamePlaceholder: "Ваше имя",
    passwordPlaceholder: "Минимум 6 символов",
    confirmPlaceholder: "Повторите пароль",
    noAccount: "Нет аккаунта?",
    hasAccount: "Уже есть аккаунт?",
    passwordsMismatch: "Пароли не совпадают",
    inviteExpired: "Ссылка-приглашение недействительна или истекла",
    inviteJoin: "Вы присоединяетесь к компании «{{company}}»",
  },
  nav: {
    dashboard: "Главная",
    dds: "ДДС",
    statements: "Выписки",
    bankAccounts: "Счета",
    analytics: "Аналитика",
    employees: "Сотрудники",
    settings: "Настройки",
    admin: "Админка",
    directory: "Справочник",
    tools: "Инструменты",
    more: "Ещё",
  },
  header: {
    notifications: "Уведомления",
    logout: "Выйти",
    lightTheme: "Светлая тема",
    darkTheme: "Тёмная тема",
    expandSidebar: "Развернуть меню",
    collapseSidebar: "Свернуть меню",
    toggleMenu: "Меню",
  },
  dashboard: {
    title: "Главная",
    welcome: "Привет",
    exportReminder: "Через {{days}} дней нужно будет сделать выгрузку",
    cardExpenses: "Траты по картам",
    myBalance: "Мой баланс",
    role: "Роль",
    owner: "Владелец",
    employee: "Сотрудник",
    balance: "Баланс",
    income: "Приход",
    expense: "Расход",
    operations: "Операции",
    balanceTimeline: "Динамика баланса",
    expensesByCategory: "Расходы по категориям",
    expenseDistribution: "Распределение расходов",
    accounts: "Счета",
    recentOperations: "Последние операции",
    "7days": "7 дней",
    "30days": "30 дней",
    "90days": "90 дней",
    "1year": "1 год",
    from: "С",
    to: "По",
    apply: "Применить",
    noData: "Нет данных",
  },
  settings: {
    entities: "ИП / Организации",
    accounts: "Остатки на счетах",
    expenses: "Категории расходов",
    templates: "Шаблоны ДДС",
    // Entities
    entityName: "Название",
    addEntity: "Добавить ИП",
    editEntity: "Редактировать ИП",
    entityPlaceholder: "Например: ИП Иванов",
    noEntities: "Нет ИП. Добавьте первое.",
    deleteEntityConfirm: "Удалить ИП? Все связанные счета и операции будут удалены.",
    accountsCount: "Счетов",
    // Accounts
    accountName: "Название счёта",
    accountType: "Тип",
    bank: "Банк",
    accountNumber: "Номер счёта",
    addAccount: "Добавить счёт",
    editAccount: "Редактировать счёт",
    noAccounts: "Нет счетов. Добавьте первый.",
    deleteAccountConfirm: "Удалить счёт?",
    initialBalances: "Остатки средств",
    balanceChecking: "Расчётные счета",
    balanceCards: "Карты и депозиты",
    balanceDate: "Дата",
    balanceAmount: "Остаток",
    balanceSaved: "Сохранено",
    addBalanceAccount: "Добавить счёт",
    noBalanceAccountsYet: "Нажмите «Добавить счёт», чтобы указать остатки",
    selectEntity: "ИП",
    selectBank: "Выберите банк",
    typeChecking: "Расчётный",
    typeCard: "Карта",
    typeCash: "Наличные",
    typeDeposit: "Депозит",
    // Expense types
    addExpenseType: "Добавить категорию",
    editExpenseType: "Редактировать категорию",
    noExpenseTypes: "Нет категорий расходов.",
    deleteTypeConfirm: "Удалить категорию? Все статьи внутри будут удалены.",
    typeName: "Название категории",
    expenseType: "Категория",
    expenseArticle: "Статья",
    entity: "ИП",
    // Articles
    addArticle: "Добавить статью",
    editArticle: "Редактировать статью",
    articleName: "Название статьи",
    noArticles: "Нет статей",
    deleteArticleConfirm: "Удалить статью?",
    // Templates
    templateName: "Название шаблона",
    addTemplate: "Добавить шаблон",
    editTemplate: "Редактировать шаблон",
    noTemplates: "Нет шаблонов. Создайте первый.",
    deleteTemplateConfirm: "Удалить шаблон?",
  },
  dds: {
    income: "Приход",
    expense: "Расход",
    transfer: "Перемещение",
    allTypes: "Все типы",
    allEntities: "Все ИП",
    allAccounts: "Все счета",
    addOperation: "Новая операция",
    editOperation: "Редактировать операцию",
    deleteOperation: "Удалить операцию",
    deleteConfirm: "Вы уверены, что хотите удалить эту операцию?",
    noOperations: "Нет операций",
    searchPlaceholder: "Поиск по комментарию или номеру...",
    userName: "Пользователь",
    date: "Дата",
    entity: "ИП",
    type: "Тип",
    from: "Откуда",
    to: "Куда",
    expenseType: "Категория",
    expenseArticle: "Статья",
    amount: "Сумма",
    comment: "Комментарий",
    orderNumber: "Номер заказа",
    operationType: "Тип операции",
    fromAccount: "Счёт списания",
    toAccount: "Счёт зачисления",
    fromTemplate: "Из шаблона",
    create: "Создать",
    confirm: "Подтвердить",
    reviewEdit: "Изменить",
    selectEntity: "Выберите ИП",
    selectType: "Тип операции",
    selectCategory: "Категория расхода",
    selectArticle: "Статья расхода",
    fillDetails: "Детали операции",
    reviewTitle: "Проверьте данные",
    exportCsv: "Экспорт CSV",
    filter: "Фильтр",
    copyInviteLink: "Скопировать ссылку",
    inviteLinkCopied: "Ссылка скопирована!",
    setupRequired: "Для работы с ДДС необходимо создать или присоединиться к компании",
    setupRequiredMember: "Для доступа к ДДС вставьте ссылку-приглашение от администратора",
    createCompanyOption: "Создать компанию",
    createCompanyDesc: "Создайте новую компанию и настройте ДДС",
    joinCompanyOption: "Присоединиться",
    joinCompanyDesc: "Вставьте ссылку-приглашение от компании",
    companyNameTaken: "Компания с таким именем уже существует",
    pasteInviteHint: "Вставьте ссылку-приглашение или токен",
    inviteLink: "Ссылка-приглашение",
    checkInvite: "Проверить",
    inviteForCompany: "Приглашение в компанию «{{company}}»",
    inviteReregisterHint: "Для присоединения необходимо зарегистрироваться по ссылке-приглашению",
  },
  pdf: {
    uploadTab: "Загрузка PDF",
    transactionsTab: "Банковские операции",
    entity: "ИП",
    account: "Счёт / карта",
    bankFormat: "Формат выписки",
    dropzone: "Нажмите или перетащите PDF-файл",
    dropzoneHint: "Поддерживаются выписки Сбер, Т-Банк, ОЗОН Банк (до 10 МБ)",
    parsing: "Разбор PDF...",
    found: "Найдено: {{count}} операций",
    duplicates: "({{count}} дубликатов)",
    duplicate: "Дубликат",
    new: "Новая",
    confirm: "Сохранить ({{count}})",
    success: "Готово!",
    savedCount: "Сохранено: {{saved}}, пропущено дубликатов: {{skipped}}",
    uploadAnother: "Загрузить ещё",
    counterparty: "Контрагент",
    purpose: "Назначение",
    balance: "Остаток",
    status: "Статус",
    allDirections: "Все направления",
    allAccounts: "Все счета",
    noTransactions: "Нет банковских операций",
    bankAccountsTab: "Расчётные счета",
    bankAccountsPlaceholder: "Раздел в разработке. Здесь будет интеграция с банковскими API для автоматической выгрузки данных.",
    uploadStatement: "Загрузить выписку",
    cardSber: "Карта Сбер",
    cardTbank: "Карта Т-Банк",
    depositTbank: "Депозит Т-Банк",
    cardOzon: "ОЗОН Банк",
    selectBank: "Выберите банк",
    selectAccount: "Выберите счёт",
    uploadFile: "Загрузите файл",
    editTransaction: "Редактировать транзакцию",
    deleteTransaction: "Удалить транзакцию",
    deleteTransactionConfirm: "Удалить эту транзакцию?",
    deleteAll: "Удалить все",
    deleteAllTitle: "Удалить все транзакции",
    deleteAllConfirm: "Удалить все транзакции ({{count}} шт.)? Это действие необратимо.",
    direction: "Направление",
    actions: "",
  },
  bankAccounts: {
    title: "Банковские счета",
    addConnection: "Подключить банк",
    editConnection: "Изменить токен",
    deleteConnection: "Удалить подключение",
    deleteConfirm: "Удалить подключение? Импортированные операции сохранятся.",
    token: "API-токен",
    tokenPlaceholder: "Вставьте токен из личного кабинета банка",
    label: "Название",
    labelPlaceholder: "Например: Т-Банк ИП Иванов",
    testConnection: "Проверить",
    testSuccess: "Подключение работает",
    testFailed: "Ошибка подключения",
    testing: "Проверка...",
    sync: "Загрузить",
    syncing: "Загрузка операций...",
    syncFrom: "С",
    syncTo: "По",
    syncResult: "Загружено: {{saved}}, пропущено: {{skipped}}",
    syncError: "Ошибка: {{error}}",
    autoSyncing: "Загрузка операций за последние 2 месяца...",
    autoSyncDone: "Автозагрузка завершена: загружено {{saved}}, пропущено {{skipped}}",
    lastSync: "Последняя загрузка",
    lastSyncNever: "Не выполнялась",
    statusSuccess: "Успешно",
    statusError: "Ошибка",
    statusPartial: "Частично",
    notConnected: "Не подключен",
    connect: "Подключить",
    noEntities: "Сначала создайте ИП в настройках",
    accounts: "Найдено счетов: {{count}}",
    tbank: "Т-Банк",
    modulbank: "Модульбанк",
    tochka: "Точка",
    viewDetails: "Подробнее",
    statusLabel: "Статус",
    localAccounts: "Обнаруженные счета",
    noLocalAccounts: "Счета ещё не обнаружены. Выполните загрузку операций.",
    transactions: "Операции",
    noTransactionsYet: "Нет операций. Выполните загрузку.",
    allAccounts: "Все счета",
  },
  employees: {
    name: "Имя",
    invite: "Пригласить сотрудника",
    inviteEmployee: "Пригласить сотрудника",
    editEmployee: "Редактировать сотрудника",
    noEmployees: "Нет сотрудников. Пригласите первого.",
    deleteConfirm: "Удалить сотрудника? Он потеряет доступ ко всем ИП.",
    entities: "ИП / Организации",
    permissions: "Права доступа",
    export: "Экспорт",
  },
  notifications: {
    empty: "Нет уведомлений",
    markRead: "Отметить как прочитанное",
    markAllRead: "Прочитать все",
    justNow: "Только что",
    minutesAgo: "{{count}} мин. назад",
    hoursAgo: "{{count}} ч. назад",
    tab: "Уведомления",
    broadcast: "Рассылка уведомлений",
    broadcastDesc: "Отправить push-уведомление всем сотрудникам компании",
    broadcastTitle: "Заголовок",
    broadcastTitlePlaceholder: "Например: Технические работы",
    broadcastBody: "Текст сообщения",
    broadcastBodyPlaceholder: "Текст уведомления...",
    broadcastSend: "Отправить всем",
    broadcastSuccess: "Уведомление отправлено",
  },
  onboarding: {
    subtitle: "Настройте вашу компанию",
    stepCompany: "Компания",
    stepEntities: "ИП",
    stepAccounts: "Счета",
    stepExpenses: "Категории",
    companyTitle: "Создайте компанию",
    companyName: "Название компании",
    companyPlaceholder: "Например: ПростоМаркет",
    createCompany: "Создать",
    companyCreated: "Компания «{{name}}» создана",
    inviteHint: "Пригласите участников (можно позже в настройках)",
    createInvite: "Создать ссылку-приглашение",
    copy: "Копировать",
    next: "Далее",
    back: "Назад",
    finish: "Завершить настройку",
    entitiesTitle: "Добавьте ИП / Организации",
    entitiesHint: "Добавьте хотя бы одно ИП или организацию",
    accountsTitle: "Добавьте счета и карты",
    accountsHint: "Добавьте хотя бы один счёт или карту для каждого ИП",
    expensesTitle: "Добавьте категории расходов",
    expensesHint: "Добавьте хотя бы одну категорию расходов",
    expensePlaceholder: "Например: Аренда, Зарплата, Реклама",
    articlePlaceholder: "Например: Доставка, Комиссия",
  },
  company: {
    title: "Компания",
    name: "Название",
    members: "Участники",
    invites: "Приглашения",
    createInvite: "Создать ссылку",
    inviteExpires: "Действует до",
    inviteUsed: "Использовано",
    inviteActive: "Активно",
    noInvites: "Нет приглашений",
    noMembers: "Нет участников",
    role: "Роль",
    owner: "Владелец",
    member: "Участник",
    employee: "Сотрудник",
    joinedViaInvite: "Присоединился по приглашению",
    entities: "ИП / Организации",
    inviteLink: "Ссылка-приглашение",
    copied: "Скопировано!",
    companyInfo: "Информация о компании",
  },
  profile: {
    title: "Профиль",
    bankIds: "Банковские реквизиты",
    bankIdsHint: "Укажите данные для автоматического сопоставления при загрузке выписок",
    sberAccount: "Номер счёта Сбер",
    sberPlaceholder: "40817810...",
    tbankCard: "Код карты Т-Банк",
    tbankCardPlaceholder: "Код из выписки",
    tbankDeposit: "Номер договора Т-Банк",
    tbankDepositPlaceholder: "Номер договора",
    saved: "Сохранено!",
    changePassword: "Смена пароля",
    currentPassword: "Текущий пароль",
    newPassword: "Новый пароль",
    changePasswordBtn: "Сменить пароль",
    passwordChanged: "Пароль успешно изменён",
    passwordError: "Ошибка смены пароля",
  },
  export: {
    downloadExcel: "Скачать Excel",
    dateFrom: "Дата от",
    dateTo: "Дата до",
    download: "Скачать",
  },
  admin: {
    title: "Админка",
    currentCompany: "Текущая компания",
    myCompanies: "Мои компании",
    switchCompany: "Переключить",
    createNewCompany: "Создать новую компанию",
    active: "Активная",
    goToSettings: "Настройки",
    members: "{{count}} участников",
    entities: "{{count}} ИП",
    noCompanies: "Нет компаний",
    switchSuccess: "Компания переключена",
  },
  directory: {
    ddsBlock: "ДДС",
    ddsBlockDesc: "Статьи, направления и счета для движения денежных средств",
    articles: "Статьи",
    articlesDesc: "Категории расходов, статьи и направления",
    accounts: "Счета",
    accountsDesc: "Расчётные счета, карты, кассы — вкл/выкл",
    directions: "Направления",
    directionsDesc: "Все направления операций",
    noCompany: "Для доступа к справочнику необходимо быть участником компании",
    noDirections: "Нет направлений",
    directionName: "Название направления",
    active: "Активен",
    enable: "Включить",
    disable: "Выключить",
    inDds: "В ДДС",
    outsideDds: "Вне ДДС",
    noEnabledAccounts: "Нет активных счетов",
    noDisabledAccounts: "Все счета активны",
    categorization: "Авторазнесение",
    categorizationDesc: "Правила автоматического разнесения банковских операций",
    addRule: "Добавить правило",
    editRule: "Редактировать правило",
    noRules: "Нет правил авторазнесения",
    rulePattern: "Паттерн",
    rulePatternPlaceholder: "Текст для поиска в операциях",
    ruleMatchField: "Где искать",
    ruleMatchCounterparty: "Контрагент",
    ruleMatchPurpose: "Назначение",
    ruleMatchAny: "Везде",
    ruleDirection: "Направление",
    ruleDirectionAll: "Любое",
    ruleDirectionIncome: "Приход",
    ruleDirectionExpense: "Расход",
    ruleCategory: "Категория",
    ruleArticle: "Статья",
    ruleDirectionName: "Направление",
    rulePriority: "Приоритет",
    deleteRuleConfirm: "Удалить правило авторазнесения?",
    accountsBlock: "Счета и карты",
    accountsBlockDesc: "Управление счетами, картами и кассами компании",
    addAccount: "Добавить поле",
    editAccount: "Редактировать",
    deleteAccountConfirm: "Удалить этот счёт? Все связанные операции будут потеряны.",
    noAccountsYet: "Нет счетов. Добавьте первый.",
    accountName: "Название",
    accountNamePlaceholder: "Напр. Расчётный в Сбере",
    accountEntity: "Юрлицо",
    accountTypePlaceholder: "Тип счёта",
    accountBankPlaceholder: "Банк",
    accountNumberPlaceholder: "Номер счёта",
    accountOtherType: "Другое...",
    accountCustomTypePlaceholder: "Введите тип",
    linkedAccount: "Привязка",
    linkAccount: "Привязать",
    unlinkAccount: "Отвязать",
    selectLinkedAccount: "Выберите счёт для привязки",
    linkedTo: "Привязан к",
    noLinkableAccounts: "Нет доступных счетов",
    addDirectionToArticle: "Добавить направление к статье...",
    initialBalance: "Остаток",
    initialBalanceDate: "Дата остатка",
    balanceFrom: "от",
  },
} as const;
